...
function NewFundraiseModal() {
  ...
  const { loadPrisonersData, loadFundraisingProjects } = useGetStoredData()
  ...
  async function saveAccountDetails(data) {
    try {
      ...
      const price = new BigNumber(data.newProjectGoal).shiftedBy(18).toString()

      await PrisonContract.newFundraise(
  ...
  const { mutateAsync: saveSinglePost } = useMutation({
    ...
    onSuccess: async () => {
      ...
      await loadFundraisingProjects(PrisonContract)
      setIsLoading(false)
    },
  })
            ...
            >
              ...
              <Formik
                ...
                validate={(values) => {
                  ...
                  if (!values.newProjectTitle) {
                    errors.newProjectTitle = "project title is empty"
                  }
                  if (!values.newProjectDescription) {
                    errors.newProjectDescription = "description is empty"
                  }
                  if (!values.newProjectGoal) {
                    errors.newProjectGoal = "enter amont to raise"
                  }
                  if (values.newProjectGoal === 0) {
                    errors.newProjectGoal = "amount is zero"
                  }
                  return errors
                }}
                  ...
                  <Form className="flex  p-6 flex-col gap-4 w-3/4 justify-center items-center text-black">
                    ...
                    <div className="flex-col gap-1 w-full">
                      <Field
                        ...
                        placeholder="prisoner description"
                        className="rounded-md h-8 w-full"
                      />
                      <ErrorMessage
                        name="newProjectDescription"
...
export default NewFundraiseModal
